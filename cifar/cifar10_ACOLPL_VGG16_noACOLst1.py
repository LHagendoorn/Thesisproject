...
import cifar10_input_VGG16_PL as cifar10_input #NOTE!!!
import VGG16base
...
tf.app.flags.DEFINE_string('skipfile','data_batch_1perc_skip.bin','File to use when skipping input') #NOTE!!!
...
def distorted_inputs(part1 = False, fileappend = '.bin', skipfile=FLAGS.skipfile):
 ...
 # print('------\nIf I didnt remove this I am probably also still using the distort based superclass labels!!!! \n BE WARNED!!! \n------')
...
def inference(images,batchS = 0, testKeep = 0, contTraining = False):
 ...
 # with tf.name_scope("content_vgg"):
  ...
  dropout1 = tf.nn.dropout(vggbase.relu7, keeprate)
    
  with tf.variable_scope('l_acol') as scope:
    acolLayers = []
    for i in range(NUM_CLASSES):
        acolLayers.append([
            _variable_with_weight_decay('weights'+str(i), shape=[4096, NUM_CLUSTERS],
...
def loss(smStacked, stackedClusts, softmaxMat, labels, superLabels,contTraining=False):
  ...
  if not contTraining: 
    print('Using tiny loss!!')
    loss = c0*cross_entropy + clust_cross_entropy
  
  #loss = tf.cond(cTrain, lambda: tf.identity(loss), lambda: tf.identity(basicLoss))
