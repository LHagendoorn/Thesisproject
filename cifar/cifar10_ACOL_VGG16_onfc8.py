...
NUM_CLASSES = 2 #cifar10_input.NUM_CLASSES
NUM_CLUSTERS = 6
...
INITIAL_LEARNING_RATE = 0.000001       # Initial learning rate. 0.1 #NOTE
...
#pikt ie het als ik dit hier doe?
selection = pd.read_csv('imagenetSelection.csv',sep=':',header=None)
selection = selection[0].values
selection = selection.astype('int32')
...
def inference(images,batchS = 0, testKeep = 0):
 ...
 # with tf.name_scope("content_vgg"):
  ...
  print(vggbase.fc8.shape)  
  fc8bits = [tf.slice(vggbase.fc8,[0,selection[i]],[batchsize,1]) for i in range(selection.size)]
  selectedFC8 = tf.squeeze(tf.stack(fc8bits,axis=1))
  print(selectedFC8.shape)

  dropout1 = tf.nn.dropout(selectedFC8, keeprate)
    
  with tf.variable_scope('l_acol') as scope:
    acolLayers = []
    for i in range(NUM_CLASSES):
        acolLayers.append([
            _variable_with_weight_decay('weights'+str(i), shape=[selection.size, NUM_CLUSTERS],
                                          stddev=0.04, wd=0.004),
            _variable_on_cpu('biases'+str(i),[NUM_CLUSTERS],tf.constant_initializer(0.1))
        ])
    clust = []
    for l in range(0,len(acolLayers)):
        clust.append(tf.matmul(dropout1, acolLayers[l][0]) + acolLayers[l][1])
...
def loss(smStacked, stackedClusts, labels, superLabels):
  ...
  c1 = tf.constant(cc1)
  c2 = tf.constant(cc2)
  c3 = tf.Variable(0.0,tf.float32)
  #c3afterSwitch = tf.constant(cc3)
  c3val = tf.constant(cc3)
  c3 = lambda affinity: tf.cond(tf.less(affinity,tresh),lambda: c3val, lambda: tf.constant(0.0))
  #switch = lambda affinity: tf.cond(tf.less(affinity,tresh),lambda: tf.constant(1),lambda: None) #Switch permenantly when below treshold

  #def f1():
  #  c3.assign(c3afterSwitch)
  #  return c3
  #def f2(): 
  #  return c3

  #c3val = lambda affinity: tf.cond(tf.less(affinity,tresh),f1,f2)
  ...
  #coacton = switch(affinity)
  #if coacton is not None:
  #  c3 = c3val
  
  #print(c3)
  #print(c3val(affinity))
  ...
  # decay terms (L2 loss).
  return loss, balance, affinity, c3(affinity)*coact
