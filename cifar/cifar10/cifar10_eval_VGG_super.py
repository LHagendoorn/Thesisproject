...
import cifar10_VGG16_PL_superclass as cifar10
...
def eval_once(saver, summary_writer, top_k_op_super, summary_op):
  """Run Eval once.
  ...
  with tf.Session() as sess:
    ...
    try:
      ...
      while step < num_iter and not coord.should_stop():
        superpredictions = sess.run([top_k_op_super])
...
def evaluate():
  ...
  with tf.Graph().as_default() as g:
    ...
    while True:
      print(superLabels)
      eval_once(saver, summary_writer, top_k_op_super, summary_op)
      if FLAGS.run_once:
        break
      time.sleep(FLAGS.eval_interval_secs)


def main(modeldir, argv=None):  # pylint: disable=unused-argument
  #print(modeldir[1])
