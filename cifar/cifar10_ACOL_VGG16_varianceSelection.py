...
#Fetch selection
with open('//code/logs/400highestvarianceFeats.pkl','rb') as f:
...
def inference(images,batchS = 0, testKeep = 0):
 ...
 # with tf.name_scope("content_vgg"):
  ...
  fc7bits = [tf.slice(vggbase.fc7,[0,selection[i]],[batchsize,1]) for i in range(selection.size)]
  selectedFC7 = tf.squeeze(tf.stack(fc7bits,axis=1))
  print(selectedFC7.shape)

  dropout1 = tf.nn.dropout(selectedFC7, keeprate)
