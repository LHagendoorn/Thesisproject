...
def train(datadir=''):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device('/cpu:0'):
        images, labels, superlabels = cifar10.distorted_inputs(datadir)
    ...
    # Calculate loss.
    loss = cifar10.loss(logits, superlabels)
...
def main(datadir = '', argv=None):  # pylint: disable=unused-argument
  ...
  if not continueTraining:
    if tf.gfile.Exists(FLAGS.train_dir):
      tf.gfile.Rename(FLAGS.train_dir,FLAGS.train_dir+str(datetime.now())+'_'+datadir[1][-16:-4])
